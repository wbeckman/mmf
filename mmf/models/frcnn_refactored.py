...
class ShapeSpec_2(namedtuple("_ShapeSpec", ["channels", "height", "width", "stride"])):
    def __new__(cls, *, channels=None, height=None, width=None, stride=None):
        return super().__new__(cls, channels, height, width, stride)


class Box2BoxTransform_2:
    """
...
class BottleneckBlock2(ResNetBlockBase):
    def __init__(
        self,
        in_channels,
        out_channels,
...
class ROIOutputs:
    def __init__(self, cfg, training=False):
        ...
        self.score_thresh = cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST
        self.min_detections = cfg.min_detections
        self.max_detections = cfg.max_detections

        nms_thresh = list(cfg.MODEL.ROI_HEADS.NMS_THRESH_TEST_OUTPUT)
...
class Res5ROIHeads_2(nn.Module):
    """
...
class GeneralizedRCNN(nn.Module):
    def __init__(self, cfg):
        ...
        self._fix_proposal_generator(cfg)
        self.roi_heads = Res5ROIHeads(cfg, self.backbone.output_shape())
        self.roi_outputs = ROIOutputs(cfg)
        self.to(self.device)

    def _fix_proposal_generator(self, cfg):
        in_channels = [
            val.channels for key, val in self.backbone.output_shape().items()
        ]
        assert len(set(in_channels)) == 1, "Each level must have the same channel!"
        in_channels = in_channels[0]
        if cfg.MODEL.PROPOSAL_GENERATOR.HIDDEN_CHANNELS == -1:
            hid_channels = in_channels
        else:
            hid_channels = cfg.MODEL.PROPOSAL_GENERATOR.HIDDEN_CHANNELS
        self.proposal_generator.rpn_head.conv = nn.Conv2d(
            in_channels, hid_channels, kernel_size=3, stride=1, padding=1
        )
        shape = self.backbone.output_shape()
        features = cfg.MODEL.RPN.IN_FEATURES
        example_head = StandardRPNHead.from_config(cfg, [shape[f] for f in features])
    ...
    def inference(
        ...
        **kwargs,
        ...
        pred_instances, _ = self.roi_heads(
            images, features, proposal_boxes
        )
        """

        predictions, features = self.forward_for_roi_head(features, proposal_boxes)

        preds_per_image = [p.size(0) for p in proposal_boxes]
        roi_features = features.split(preds_per_image, dim=0)
